...
impl Fq12 {
    ...
    pub fn cyclotomic_squared(&self) -> Self {
        ...
        t0.add_inp(&z1);
        let mut t0_rhs = z1;
        t0_rhs.mul_by_nonresidue();
        t0_rhs.add_inp(&z0);
        t0.mul_inp(&t0_rhs);
        t0.sub_inp(&tmp);
        let mut tmp_nonresidue = tmp;
        tmp_nonresidue.mul_by_nonresidue();
        t0.sub_inp(&tmp_nonresidue);
        ...
        t2.add_inp(&z3);
        let mut t2_rhs = z3;
        t2_rhs.mul_by_nonresidue();
        t2_rhs.add_inp(&z2);
        t2.mul_inp(&t2_rhs);
        t2.sub_inp(&tmp);
        let mut tmp_nonresidue = tmp;
        tmp_nonresidue.mul_by_nonresidue();
        t2.sub_inp(&tmp_nonresidue);
        ...
        t4.add_inp(&z5);
        let mut t4_rhs = z5;
        t4_rhs.mul_by_nonresidue();
        t4_rhs.add_inp(&z4);
        t4.mul_inp(&t4_rhs);
        t4.sub_inp(&tmp);
        let mut tmp_nonresidue = tmp;
        tmp_nonresidue.mul_by_nonresidue();
        t4.sub_inp(&tmp_nonresidue);
